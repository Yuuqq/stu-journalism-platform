...
复制提示词到 Claude
通过 Claude 接入，或自行部署在本地环境（详细见下方）。
步骤 1：输入个人信息：姓名、求职岗位、简历内容等。
步骤 2：模型自动提炼个人优势并生成简历草稿。
步骤 3：用户可选择修改、调整简历或更换配色方案。
步骤 4：最终生成 A4 竖版 PDF 简历，用户可以下载。
社交名片制作
复制下面的提示词
...
使用方法
复制提示词到GPT，Claude 等大模型
将你的个人介绍，个人头像的链接（用图床上传获得图片链接），个人主页的链接提供给 AI
通义AI 会生成卡片代码。
复制卡片代码，保存为 html 文件，然后直接打开，就可以看到生成后的图片。（或者把代码复制到 https://www.toolhelper.cn/Html/Preview 这个网站显示也可）
项目特性
角色定位：智能简历生成工具，能够根据用户提供的简历信息，自动提炼个人亮点与核心竞争力，生成符合HR审美的精美简历。
自动分析与生成：通过自然语言处理模型，分析用户简历、岗位需求等内容，自动输出简历中的关键信息，突出优势，如：名校背景、金牌实习、团队精神等。
多种配色方案：简历自动生成时，提供十种柔和设计感的配色方案，随机选择，确保简历设计精美。
完全响应式：自动调整布局以适应不同设备显示。
使用A4纸张比例：简历输出时始终保持A4纸张竖版比例，保证打印效果。
技术实现
该项目基于以下技术栈：
...
npm start
打开浏览器，访问 http://localhost:3000，即可使用简历生成器。
